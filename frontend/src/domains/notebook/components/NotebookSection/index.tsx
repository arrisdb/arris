...
import { NOTEBOOK_TABS_COLLAPSED_KEY } from "./constants";
import { notebookContextMenuItems } from "./utils";

function NotebookSection() {
  const allTabs = useTabsStore((s) => s.tabs);
  const tabs = useMemo(
    () => allTabs.filter((t) => t.tabType === "notebook" && !t.filePath),
    [allTabs],
  );
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    ...
    return window.localStorage.getItem(NOTEBOOK_TABS_COLLAPSED_KEY) === "1";
  });
  ...
  const ctxItems: ContextMenuItem[] = ctxMenu.state
    ? notebookContextMenuItems({
        tabId: ctxMenu.state.context,
        onRename: startRename,
        onDelete: softDelete,
        onMoveToProject: moveToProject,
        onNewNotebook: () => openUntitledNotebookTab(),
      })
    : [];
    ...
    >
      {!collapsed && (
        <div
          ...
          data-testid="notebook-section-resizer"
      ...
      <button
        ...
        data-testid="notebook-section-toggle"
      ...
      {!collapsed && (
        <div
          ...
          data-testid="notebook-section-list"
          onContextMenu={(e) => {
            if ((e.target as HTMLElement).closest("[data-notebook-row]")) return;
            ctxMenu.open(e, null);
          }}
        >
          {sortedTabs.map((t) => {
            ...
            if (isSoftDeleted) {
              return (
                <div
                  ...
                  data-testid={`notebook-section-row-${t.id}`}
                >
                  ...
                  <button
                    ...
                    data-testid={`notebook-section-restore-${t.id}`}
                  >
                    Restore
                  </button>
                </div>
              ...
              >
                {isRenaming ? (
                  <input
                    ...
                    data-testid={`notebook-section-rename-${t.id}`}
                  />
                ) : (
                  <span className="name">{t.title}</span>
                )}
                <button
                  ...
                  title="Delete tab"
                  data-testid={`notebook-section-delete-${t.id}`}
                >
      ...
      {ctxMenu.state && (
        <ContextMenu
          ...
          data-testid="notebook-section-ctx-menu"
        />
      )}
    </div>
  );
