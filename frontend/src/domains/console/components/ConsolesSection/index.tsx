...
import { CONSOLES_COLLAPSED_KEY } from "./constants";
import { consoleContextMenuItems } from "./utils";

function ConsolesSection() {
  const allTabs = useTabsStore((s) => s.tabs);
  const tabs = useMemo(
    () => allTabs.filter((t) => (!t.tabType || t.tabType === "console") && !t.filePath),
    [allTabs],
  );
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    ...
    return window.localStorage.getItem(CONSOLES_COLLAPSED_KEY) === "1";
  });
    ...
    >
      ...
      >
        <Icon
          ...
          size={11}
        />
        <span>SQL Consoles</span>
      </button>
      {!collapsed && (
        <div
          className="mdbc-consoles-list"
          data-testid="consoles-list"
          onContextMenu={(e) => {
            if ((e.target as HTMLElement).closest("[data-console-row]")) return;
            ctxMenu.open(e, null);
          }}
        >
          {sortedTabs.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    data-testid={`consoles-restore-${t.id}`}
                  >
                    Restore
                  </button>
                </div>
              ...
              >
                {isRenaming ? (
                  <input
                    ...
                    data-testid={`consoles-rename-${t.id}`}
                  />
                ) : (
                  <span className="name">{t.title}</span>
                )}
                <button
                  ...
                  title="Delete tab"
                  data-testid={`consoles-delete-${t.id}`}
                >
